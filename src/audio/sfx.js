...
class SfxManager {
    ...
    async playImmediate(path, volume = 0.5) {
        if (!this.ctx) {
    ...
    loadSfxr(key, b58) {
        ...
        for (let i = 0; i < samples.length; i++)
            channel[i] = (samples[i] - 128) / 128;
        this.buffers[key] = buf;
    }
    /** Play a sound effect. Uses Web Audio API for instant playback. */
    play(key) {
        if (this._muted || !this.ctx || !this.gainNode)
            return;
    ...
    playPitched(key, volume, rate, startOffset = 0) {
        if (this._muted || !this.ctx || !this.gainNode)
            return;
        const buf = this.buffers[key];
        if (!buf)
    ...
    toggle() {
        ...
        if (this.bgmGain)
            this.bgmGain.gain.value = this._muted ? 0 : this._bgmVolume;
    }
    // ---- Background music ----
    async loadBgm(path) {
        try {
    ...
    playBgm() {
        if (!this.ctx || !this.bgmGain || !this.bgmBuffer)
            return;
        this.stopBgm();
        if (this.ctx.state === 'suspended')
            this.ctx.resume();

...
export class Tower extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, tileX, tileY, kind = 'arrow') {
        ...
        this.level = 0;
        this.lastShot = 0;
        this.stand = null; // static ballista stand (arrow only)
        this.nockedArrow = null; // real arrow resting on bow (arrow only)
        this.size = CFG.tower.tiles;
        this.setScale(0.5);
    ...
    upgrade() {
        ...
        if (this.stand)
            targets.push(this.stand);
        if (this.nockedArrow)
            targets.push(this.nockedArrow);
    ...
    applyTierVisual() {
        ...
        if (this.nockedArrow) {
            ...
            if (arrowTint)
                this.nockedArrow.setTint(arrowTint);
            else
                this.nockedArrow.clearTint();
        }
    ...
    hurt(amount) {
        ...
        if (this.stand)
            this.stand.setTintFill(0xffffff);
        if (this.nockedArrow)
            this.nockedArrow.setTintFill(0xffffff);

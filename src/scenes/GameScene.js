...
import { createSparseGrid, findPath, canReachFromSpawnDirections, gridGet, gridSet } from '../systems/Pathfinding';
...
export class GameScene extends Phaser.Scene {
    constructor() {
        ...
        this.waveStartAt = 0;
        this.wave = 0; // 0-indexed current wave
        this.waveSpawned = 0; // enemies spawned in the current wave
        this.waveKills = 0; // kills counted for the current wave
        this.pathsThisFrame = 0; // BFS pathfinding budget per frame
        this.waveBreakUntil = 0; // timestamp when the inter-wave build break ends
        this.countdownMsg = '';
        ...
        this.towerPanelBounds = { x: 0, y: 0, w: 0, h: 0 };
        this.towerIndicators = new Map();
        this.gapBlockers = null;
        this.sellTimers = new Map();
        this.bossIndicator = null;
        this.boss = null;
        this.bossSpawned = false;
        this.bossCountdownUntil = 0; // set once the last wave is cleared, boss spawns at this time
        // Castle multi-boss state
        this.castlePhase = 0; // 0 = waves 1-2, 1 = queen boss, 2 = waves 3-4, 3 = dragon boss
        this.midBoss = null;
        ...
        this.levelRampFactor = CFG.spawn.rampFactor;
        this.levelMinInterval = CFG.spawn.minInterval;
        this.levelWaveSize = CFG.spawn.waveSize;
        this.levelClusterMax = 4;
        this.boulders = [];
        this.webs = [];
        this.gasClouds = [];
        this.birdPoops = [];
        ...
        this.treeSeed = 0;
        this.sf = 1; // native resolution scale factor
        this._wallCheckCache = { key: '', valid: false };
        this._lastWallCheckPlayerTile = '';
    ...
    create() {
        ...
        canvas.width = CFG.tile * 2;
        canvas.height = CFG.tile;
        ...
        this.wallLayer = this.wallTilemap.createBlankLayer('walls', tileset, -(mapSize / 2) * CFG.tile, -(mapSize / 2) * CFG.tile);
        ...
        else if (this.difficulty === 'hard' || this.difficulty === 'oneHP')
        ...
        this.physics.add.collider(this.enemies, this.wallGroup, (e, w) => this.enemyHitsWall(e, w), (_e, _w) => !(_e.flying));
        this.physics.add.collider(this.enemies, this.towerGroup, (e, t) => this.enemyHitsTower(e, t), (_e, _t) => !(_e.flying));
        this.physics.add.overlap(this.player, this.enemies, (_p, e) => this.enemyHitsPlayer(e));
        this.physics.add.overlap(this.projectiles, this.enemies, (pr, en) => this.projectileHitsEnemy(pr, en));
        this.physics.add.overlap(this.player, this.enemyDarts, (_p, d) => this.enemyDartHitsPlayer(d));
        this.physics.add.overlap(this.enemyDarts, this.wallGroup, (_d, _w) => { _d.destroy(); });
        this.physics.add.overlap(this.enemyDarts, this.towerGroup, (_d, _t) => { _d.destroy(); });
        // Toad globs hit the player (they arc over walls/towers — no wall/tower overlap)
        this.physics.add.overlap(this.player, this.toadGlobs, (_p, g) => this.toadGlobHitsPlayer(g));
        // Castle warlock bolts, queen orbs, dragon fireballs
        this.physics.add.overlap(this.player, this.warlockBolts, (_p, b) => this.castleBoltHitsPlayer(b));
        this.physics.add.overlap(this.warlockBolts, this.wallGroup, (_b, _w) => { _b.destroy(); });
        this.physics.add.overlap(this.player, this.queenOrbs, (_p, o) => this.castleBoltHitsPlayer(o));
        this.physics.add.overlap(this.player, this.dragonFireballs, (_p, f) => this.dragonFireballHitsPlayer(f));
        this.physics.add.overlap(this.dragonFireballs, this.wallGroup, (_f, _w) => this.dragonFireballExplode(_f));
        this.physics.add.overlap(this.dragonFireballs, this.towerGroup, (_f, _t) => this.dragonFireballExplode(_f));
        ...
        this.input.keyboard.on('keydown-ONE', () => {
            ...
            if (ts && ts !== 'game_press_1' && ts !== 'game_place_tower')
                return;
            this.toggleBuild('tower', 'arrow');
        });
        this.input.keyboard.on('keydown-TWO', () => {
            if (this.game.registry.get('tutorialStep'))
                return; // blocked during tutorial
            this.toggleBuild('tower', 'cannon');
        });
        // Key 3 reserved for mage tower (not yet implemented)
        this.input.keyboard.on('keydown-FOUR', () => {
            const ts = this.game.registry.get('tutorialStep');
            if (ts && ts !== 'game_press_4' && ts !== 'game_place_walls')
                return;
        ...
        this.game.events.on('ui-build', (k, tk) => {
            const ts = this.game.registry.get('tutorialStep');
            if (ts) {
                if (ts === 'game_press_1' && k === 'tower' && tk === 'arrow') { /* allowed */ }
                    ...
                    return; // block everything else during tutorial
            }
            this.toggleBuild(k, tk);
        });
        this.game.events.on('ui-sell', () => this.setBuild('none'));
        this.game.events.on('ui-speed', (mult) => this.setTimeScale(mult));
        ...
        if (this.biome === 'forest') {
            ...
            fireflyEmitter.addEmitZone({ type: 'random', source: new Phaser.Geom.Rectangle(-400, -300, 800, 600) });
        ...
        if (this.biome === 'infected') {
            ...
            sporeEmitter.addEmitZone({ type: 'random', source: new Phaser.Geom.Rectangle(-500, -400, 1000, 800) });
            ...
            sporeGreenEmitter.addEmitZone({ type: 'random', source: new Phaser.Geom.Rectangle(-500, -400, 1000, 800) });
        }
    ...
    setBuild(k, towerKind) {
        this.buildKind = k;
        if (k === 'tower' && towerKind)
            this.buildTowerKind = towerKind;
        this.ghost.setVisible(k !== 'none');
        if (this.gridOverlay)
            this.gridOverlay.setVisible(k !== 'none');
    ...
    canPlaceTower(tx, ty) {
        ...
        for (let j = 0; j < s; j++) {
            for (let i = 0; i < s; i++) {
                if (gridGet(this.grid, tx + i, ty + j) !== 0)
                    return false;
            }
        }
    ...
    handleClick(p) {
        ...
        if (this.selectedTower && this.pointInPanel(wx, wy))
            return;
        ...
        if (this.keys.X.isDown) {
            if (this.game.registry.get('tutorialActive'))
                return; // no selling during tutorial
            this.sellAt(tx, ty);
            return;
        }
        // click an existing tower with no active build = select it
        if (this.buildKind === 'none') {
            const hit = this.towers.find(t => tx >= t.tileX && tx < t.tileX + t.size &&
        ...
        if (this.buildKind === 'tower') {
            ...
            if (!this.canPlaceTower(ox, oy))
                return;
            const kindCost = CFG.tower.kinds[this.buildTowerKind].cost;
            if (this.player.money < kindCost)
                return;
            ...
            for (let j = 0; j < s; j++)
                for (let i = 0; i < s; i++)
                    gridSet(this.grid, ox + i, oy + j, 2);
            this.gridVersion++;
            this._wallCheckCache = { key: '', valid: false };
            this.rebuildGapBlockers();
            this.rebuildGapBlockers();
            this.pushHud();
            SFX.play('towerPlace');
            if (this.game.registry.get('tutorialActive'))
                this.game.events.emit('tutorial-tower-placed');
            this.setBuild('none');
            return;
        }
        // wall
        if (gridGet(this.grid, tx, ty) !== 0)
            return;
        if (this.player.money < CFG.wall.cost)
            return;
        const pt = this.worldToTile(this.player.x, this.player.y);
        ...
        if (afterReach < Math.min(beforeReach, 2))
            return;
        ...
        SFX.play('wallPlace');
        if (this.game.registry.get('tutorialActive'))
            this.game.events.emit('tutorial-wall-placed');
    }
    sellAt(tx, ty) {
        if (this.game.registry.get('tutorialActive'))
            return;
        // tower: click anywhere inside the footprint
        const ti = this.towers.findIndex(t => tx >= t.tileX && tx < t.tileX + t.size &&
            ty >= t.tileY && ty < t.tileY + t.size);
        if (ti >= 0) {
    ...
    buildTowerPanel(t) {
        ...
        const stats = this.add.text(-W / 2 + 8, -H / 2 + 22, `DMG ${st.damage}  RNG ${st.range}${splashLine}\nFIRE ${(1000 / st.fireRate).toFixed(1)}/s  HP ${t.hp}/${t.maxHp}`, { fontFamily: 'monospace', fontSize: '10px', color: '#ccd' }).setResolution(tr);
        panel.add(stats);
        // Upgrade button
        ...
        if (canUp) {
            ...
            upBg.on('pointerdown', (_p, _lx, _ly, ev) => {
                ev?.stopPropagation?.();
        ...
        sellBg.on('pointerdown', (_p, _lx, _ly, ev) => {
            ev?.stopPropagation?.();
            if (this.game.registry.get('tutorialActive'))
                return;
            this.doSellSelected();
    ...
    hasLineOfSight(x1, y1, x2, y2) {
        ...
        while (true) {
            ...
            if (v === 1 || v === 2)
                return false; // wall or tower blocks
            const e2 = err * 2;
            if (e2 > -dy) {
                err -= dy;
                tx += sx;
    ...
    countReachableDirections(px, py) {
    ...
    generateChunksAround(wx, wy, force = false) {
        ...
        if (!force && cx === this.lastChunkCx && cy === this.lastChunkCy)
            return;
        this.lastChunkCx = cx;
        this.lastChunkCy = cy;
        ...
        this.pendingChunks.sort((a, b) => ((a.cx - cx) ** 2 + (a.cy - cy) ** 2) - ((b.cx - cx) ** 2 + (b.cy - cy) ** 2));
    ...
    processChunkQueue(budgetMs) {
        ...
        while (this.pendingChunks.length > 0 && processed < maxPerFrame) {
            ...
            if (budgetMs > 0 && performance.now() - start >= budgetMs)
                break;
            ...
            if (this.biome === 'forest' || this.biome === 'infected')
                this.placeTreesInChunk(ccx, ccy);
            // Generate river terrain blockers
            if (this.biome === 'river')
                this.placeRiverInChunk(ccx, ccy);
            processed++;
        }
    }
    ...
    update(_realTime, delta) {
        ...
        if (this.biome === 'river')
            this.updateRiverSquiggles(delta);
    ...
    updateDepthSort() {
        ...
        for (const tower of this.towers) {
            ...
            tower.setDepth(d);
            if (tower.stand)
                tower.stand.setDepth(d + 0.1);
            tower.top.setDepth(d + 0.2);
            if (tower.nockedArrow)
                tower.nockedArrow.setDepth(d + 5);
        }
        // Enemies
    ...
    rebuildGapBlockers() {
        ...
        for (const [key] of this.grid) {
            ...
            for (const [cx, cy] of [[kx, ky], [kx + 1, ky], [kx, ky + 1], [kx + 1, ky + 1]]) {
                ...
                blocker.body.setSize(size, size);
                blocker.body.position.set(wx - size / 2, wy - size / 2);
                this.gapBlockers.add(blocker);
            }
        }
    }
    /** Destroy a tree tile at grid coords, removing its blocker and sprite. */
    destroyTreeTile(gx, gy) {
        const t = CFG.tile;
        ...
        for (const child of this.wallGroup.getChildren()) {
            if (Math.abs(child.x - wx) < 2 && Math.abs(child.y - wy) < 2) {
                child.destroy();
                break;
            }
        }
    ...
    placeRiverInChunk(cx, cy) {
        const chunkKey = `${cx},${cy}`;
        if (this.riverChunksGenerated.has(chunkKey))
            return;
    ...
    placeTreesInChunk(cx, cy) {
        const chunkKey = `${cx},${cy}`;
        if (this.treeChunksGenerated.has(chunkKey))
            return;
        ...
        while (placed < clustersPerChunk && attempts < maxAttempts) {
            ...
            for (const tile of pattern.tiles) {
                ...
                this.physics.add.existing(blocker, true);
                blocker.body.setSize(t, t);
                blocker.body.position.set(wx - t / 2, wy - t / 2);
    ...
    updatePlayer(time, _delta) {
        ...
        if (this.game.registry.get('tutorialActive') && this.towers.length > 0) {
            ...
            let clamped = false;
            if (this.player.x > anchor.x + maxDx) {
                this.player.x = anchor.x + maxDx;
                clamped = true;
            }
            if (this.player.x < anchor.x - maxDx) {
                this.player.x = anchor.x - maxDx;
            ...
            if (this.player.y > anchor.y + maxDy) {
                this.player.y = anchor.y + maxDy;
                clamped = true;
            }
            if (this.player.y < anchor.y - maxDy) {
                this.player.y = anchor.y - maxDy;
            ...
            if (clamped) {
                ...
                if (this.player.x === anchor.x + maxDx || this.player.x === anchor.x - maxDx)
                    body.velocity.x = 0;
                if (this.player.y === anchor.y + maxDy || this.player.y === anchor.y - maxDy)
                    body.velocity.y = 0;
            }
        }
        ...
        if (target) {
            ...
            bow.setPosition(this.player.x + Math.cos(aimAngle) * offset, this.player.y + 2 + Math.sin(aimAngle) * offset);
            ...
            if (st.splashRadius > 0) {
                ...
                if (!tgt) {
                    ...
                    if (time > tower.lastShot + 150) {
                        ...
                        if (tower.nockedArrow)
                            tower.nockedArrow.setVisible(true);
                    }
                    // Keep the nocked arrow aligned with the bow's current rotation while idle
                    if (tower.nockedArrow) {
                        tower.nockedArrow.setPosition(tower.top.x + Math.cos(tower.top.rotation) * 23, tower.top.y + Math.sin(tower.top.rotation) * 23);
                ...
                if (time > tower.lastShot + st.fireRate) {
                    ...
                    if (tower.nockedArrow)
                        tower.nockedArrow.setVisible(false);
                ...
                if (tower.nockedArrow) {
                    tower.nockedArrow.setPosition(tower.top.x + Math.cos(angle) * 23, tower.top.y + Math.sin(angle) * 23);
    ...
    findBestCannonTarget(tx, ty, range, splash, projSpeed) {
    ...
    findMostThreateningEnemy(x, y, range) {
        let best = null;
        ...
        this.enemies.children.iterate((c) => {
            ...
            if (eucD2 > r2)
                return true; // out of shooting range
    ...
    updateEnemies(time, _delta) {
        this.enemies.children.iterate((c) => {
            ...
            if (e.kind === 'mosquito') {
                ...
                if (dist < meleeRange) {
                    e.setVelocity(0, 0);
                    if (e.anims.currentAnim?.key !== `${prefix}-atk`)
                        e.play(`${prefix}-atk`);
                    if (time > e.attackCd) {
                        e.attackCd = time + 700;
                ...
                if (e.anims.currentAnim?.key !== `${prefix}-move`)
                    e.play(`${prefix}-move`);
                return true;
            }
            ...
            if (e.kind === 'toad') {
                ...
                if (dist < toadRange && dist > 40) {
                    ...
                    if (!td._toadHopping) {
                        e.setVelocity(0, 0);
                        if (e.anims.currentAnim?.key !== 'etd-idle')
                            e.play('etd-idle');
                    }
            ...
            if (dist2 < 30 * 30) {
                ...
                if (e.rotates) {
                    ...
                    if (dirChanged || e.anims.currentAnim?.key !== atkAnim)
                        e.play(atkAnim);
                }
                else {
                    e.setFlipX(tx - e.x < 0);
            ...
            if (clear) {
                ...
                if (e.rotates) {
                    ...
                    if (dirChanged || e.anims.currentAnim?.key !== moveAnim)
                        e.play(moveAnim);
                }
                else {
                    e.setFlipX(dx < 0);
            ...
            if (e.path && e.path.length > 0) {
                ...
                for (let i = e.path.length - 1; i > e.pathIdx; i--) {
                    ...
                    if (!this.lineBlocked(e.x, e.y, nx, ny)) {
                        lookahead = i;
                        break;
                    }
                }
                ...
                if (d < 4 && e.pathIdx < e.path.length - 1)
                    e.pathIdx++;
                if (d > 0.01) {
                    moveX = dx / d;
                    moveY = dy / d;
    ...
    updateBoss(time) {
        ...
        if (b.state === 'charging') {
            ...
            if (!chargeHit) {
                for (const w of this.walls) {
                    if (Phaser.Math.Distance.Between(b.x, b.y, w.x, w.y) < 40) {
                        chargeHit = true;
                        break;
                    }
                }
        ...
        if (clear) {
            ...
            if (b.path && b.path.length > 0) {
                ...
                const d = Math.hypot(dx, dy);
                if (d < 4 && b.pathIdx < b.path.length - 1)
                    b.pathIdx++;
                if (d > 0.01) {
                    moveX = dx / d;
                    moveY = dy / d;
        ...
        if (b.anims.currentAnim?.key !== moveAnim)
            b.play(moveAnim);
        // passive contact damage (touching the player)
        if (Phaser.Math.Distance.Between(b.x, b.y, this.player.x, this.player.y) < 36 &&
            time > b.contactCd) {
    ...
    spawnGasCloud(x, y) {
        ...
        for (const gc of this.gasClouds) {
            if (Phaser.Math.Distance.Between(x, y, gc.x, gc.y) < 20)
                return;
        }
    ...
    spawnBirdPoop(x, y) {
        ...
        for (const bp of this.birdPoops) {
            if (Phaser.Math.Distance.Between(x, y, bp.sprite.x, bp.sprite.y) < 24)
                return;
        }
        // Cap total poops on screen
        if (this.birdPoops.length >= 30)
            return;
    ...
    spawnBoss() {
        ...
        this.physics.add.overlap(this.projectiles, this.boss, (a, b) => {
            const pr = (a instanceof Projectile ? a : b);
            const bs = (a instanceof Boss ? a : b);
    ...
    spawnCastleBoss(kind) {
        ...
        if (kind === 'queen') {
            b.hp = CFG.castle.queenHp;
            b.maxHp = CFG.castle.queenHp;
            b.dmg = CFG.castle.queenDmg;
            b.speed = CFG.castle.queenSpeed;
            this.midBoss = b;
            this.castlePhase = 1;
            ...
            b.hp = CFG.castle.dragonHp;
            b.maxHp = CFG.castle.dragonHp;
            b.dmg = CFG.castle.dragonDmg;
            b.speed = CFG.castle.dragonSpeed;
            this.castlePhase = 3;
            this.nextDragonFireball = this.vTime + CFG.castle.dragonFireballRate;
        }
        this.boss = b;
        this.pushHud();
        this.physics.add.overlap(this.projectiles, b, (a, bb) => {
            const pr = (a instanceof Projectile ? a : bb);
            const bs = (a instanceof Boss ? a : bb);
            this.projectileHitsBoss(pr, bs);
        });
    ...
    enemyHitsTower(_e, _t) {
        // Enemies no longer attack structures — collider just blocks movement
    }
    // ---------- ENEMY DARTS (mosquito ranged attack) ----------
    spawnMosquitoDart(x, y, tx, ty) {
        ...
        dart._born = this.vTime;
        dart._dmg = CFG.river.mosquitoDartDmg;
    }
    enemyDartHitsPlayer(dart) {
        if (!dart.active)
            return;
        const dmg = dart._dmg ?? CFG.river.mosquitoDartDmg;
        this.player.hurt(dmg, this);
        this.pushHud();
        if (this.player.hp <= 0)
            this.lose();
    ...
    updateEnemyDarts() {
        this.enemyDarts.children.iterate((c) => {
            if (!c || !c.active)
                return true;
            if (this.vTime - c._born > CFG.river.mosquitoDartLifetime) {
                c.destroy();
                return true;
            }
            return true;
    ...
    isAdjacentToObstacle(wx, wy) {
        ...
        for (let dy = -1; dy <= 1; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
                ...
                if (v === 1 || v === 2 || v === 3 || v === 4)
                    return true; // wall, tower, tree, water
            }
        }
        return false;
    }
    spawnToadGlob(x, y, tx, ty, blockedByAdjacent) {
        ...
        if (blockedByAdjacent) {
            this.physics.add.overlap(glob, this.wallGroup, () => { if (!g._landed)
                this.landToadGlob(glob); });
            this.physics.add.overlap(glob, this.towerGroup, () => { if (!g._landed)
                this.landToadGlob(glob); });
        }
    }
    ...
    toadGlobHitsPlayer(glob) {
        // Player overlap is only used as a fallback — main landing is in updateToadGlobs
        if (!glob.active || glob._landed)
            return;
        this.landToadGlob(glob);
    }
    updateToadGlobs() {
        this.toadGlobs.children.iterate((c) => {
            if (!c || !c.active || c._landed)
                return true;
            const elapsed = this.vTime - c._born;
            if (elapsed > CFG.infected.toadGlobLifetime) {
                this.landToadGlob(c);
                return true;
            }
    ...
    spawnWarlockBolt(sx, sy, tx, ty) {
        ...
        bolt._born = this.vTime;
        bolt._dmg = CFG.castle.warlockBoltDmg;
    }
    spawnQueenOrb(sx, sy, tx, ty) {
        ...
        orb._born = this.vTime;
        orb._dmg = CFG.castle.queenOrbDmg;
    }
    spawnDragonFireball(sx, sy, tx, ty) {
        ...
        fb._born = this.vTime;
        fb._dmg = CFG.castle.dragonFireballDmg;
        fb._splash = CFG.castle.dragonFireballSplash;
        fb._tx = tx;
        fb._ty = ty;
        fb._totalDist = d;
        fb._startX = sx;
    ...
    castleBoltHitsPlayer(bolt) {
        if (!bolt.active)
            return;
        const dmg = bolt._dmg ?? 6;
        this.player.hurt(dmg, this);
        this.pushHud();
        if (this.player.hp <= 0)
            this.lose();
    ...
    dragonFireballHitsPlayer(fb) {
        if (!fb.active)
            return;
        const dmg = fb._dmg ?? 15;
    ...
    dragonFireballExplode(fb) {
        if (!fb.active)
            return;
        const x = fb.x, y = fb.y;
        const dmg = fb._dmg ?? 15;
        const splash = fb._splash ?? 48;
        fb.destroy();
        // Explosion animation
    ...
    updateCastleProjectiles() {
        // Warlock bolts
        this.warlockBolts.children.iterate((c) => {
            if (!c || !c.active)
                return true;
            if (this.vTime - c._born > CFG.castle.warlockBoltLifetime) {
                c.destroy();
                return true;
            }
            return true;
        ...
        // Queen orbs
        this.queenOrbs.children.iterate((c) => {
            if (!c || !c.active)
                return true;
            if (this.vTime - c._born > CFG.castle.queenOrbLifetime) {
                c.destroy();
                return true;
            }
            return true;
        ...
        const fbToExplode = [];
        this.dragonFireballs.children.iterate((c) => {
            if (!c || !c.active)
                return true;
    ...
    // ---------- PROJECTILES ----------
    spawnProjectile(x, y, tx, ty, speed, dmg, splashRadius = 0, scale = 0.5, tint = 0, homingTarget = null) {
    ...
    updateProjectiles(time) {
        this.projectiles.children.iterate((c) => {
            ...
            if (p.homingTarget && !p.groundTarget) {
                if (p.homingTarget.active && !p.homingTarget.dying) {
    ...
    projectileHitsBoss(pr, b) {
        if (!pr.active || !b.active || b.dying)
            return;
    ...
    projectileHitsEnemy(pr, e) {
        if (!pr.active || !e.active || e.dying)
            return;
        // Cannonballs ignore direct hits — they explode on reaching their ground target
        if (pr.groundTarget)
            return;
    ...
    applyDamageToEnemy(e, dmg) {
        ...
        if (e.hp <= 0) {
            if (!e.noCoinDrop) {
                const tier = e.kind === 'heavy' || e.kind === 'deer' || e.kind === 'bear' || e.kind === 'infected_heavy' ? 'silver' :
                    'bronze';
            ...
            this.pushHud();
            if (this.game.registry.get('tutorialActive'))
                this.game.events.emit('tutorial-kill');
        }
    }
    cannonExplode(x, y, radius, dmg) {
        SFX.play('boom');
        // ---------- VISUALS ----------
    ...
    spawnRunnerPack() {
        ...
        let cx = 0, cy = 0;
        if (side === 0) {
            cx = px + Phaser.Math.Between(-spawnR, spawnR);
            cy = py - spawnR;
        }
        if (side === 1) {
            cx = px + Phaser.Math.Between(-spawnR, spawnR);
        ...
        if (side === 2) {
            cx = px - spawnR;
            cy = py + Phaser.Math.Between(-spawnR, spawnR);
        }
        if (side === 3) {
            cx = px + spawnR;
            cy = py + Phaser.Math.Between(-spawnR, spawnR);
    ...
    spawnEnemy() {
        ...
        if (this.biome === 'forest') {
            ...
            const r = Math.random();
            if (r < CFG.infected.toadChance)
                kind = 'toad';
            else if (r < CFG.infected.toadChance + this.heavyChance)
                kind = 'infected_heavy';
            else
                kind = 'infected_basic';
        }
            ...
            if (r < this.heavyChance)
                kind = 'golem';
            else if (r < this.heavyChance + 0.15)
                kind = 'warlock';
            else if (r < this.heavyChance + 0.35)
                kind = 'shadow_imp';
            else
                kind = 'skeleton';
        }

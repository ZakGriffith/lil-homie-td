...
import { LEVELS, DIFFICULTY_ORDER, DIFFICULTY_LABELS, MEDAL_COLORS, BIOME_COLORS, loadMedals, totalMedals, isLevelUnlocked } from '../levels';
...
export class LevelSelectScene extends Phaser.Scene {
    ...
    drawBiomeIcon(g, cx, cy, biome) {
        ...
        switch (biome) {
            ...
            case 'tundra':
                ...
                for (let i = 0; i < 3; i++) {
                    const a = (i / 3) * Math.PI;
                    g.lineBetween(cx + Math.cos(a) * 7 * s, cy + Math.sin(a) * 7 * s, cx - Math.cos(a) * 7 * s, cy - Math.sin(a) * 7 * s);
                }
    ...
    drawNodes() {
        for (const level of LEVELS) {
            ...
            hitZone.on('pointerdown', () => {
                ...
                if (this.game.registry.get('tutorialStep') === 'ls_click_meadow' && level.id !== 1)
                    return;
                if (!unlocked) {
    ...
    openDifficultyPanel(level) {
        ...
        backdrop.on('pointerdown', () => {
            if (this.game.registry.get('tutorialActive'))
                return; // don't close during tutorial
        ...
        const items = [backdrop, outerBox, innerBox, title, tag, divider];
        for (let i = 0; i < DIFFICULTY_ORDER.length; i++) {
            const diff = DIFFICULTY_ORDER[i];
            ...
            hitRect.on('pointerover', () => {
                if (this.game.registry.get('tutorialActive') && diff !== 'easy')
                    return;
                btnBg.clear();
    ...
    updateStartButton() {
        if (!this.startBtnG || !this.startHit || !this.startText)
            return;
    ...
    closeDifficultyPanel() {
        ...
        for (const obj of this.panelExtras)
            obj.destroy();
        this.panelExtras = [];
        this.selectedLevel = null;
        this.selectedDiff = null;
    ...
    startMission() {
        if (!this.selectedLevel || !this.selectedDiff)
            return;

...
export class TutorialScene extends Phaser.Scene {
    ...
    constructor() {
        ...
        this.sf = 1;
        this.hudLabels = [];
        this.hudClickZone = null;
        this.continueZone = null;
        // Tracking
        this.moveDist = 0;
        this.lastPx = 0;
        this.lastPy = 0;
        ...
        this.watchTimer = 0;
        this.stepDelay = 0;
        this.onLevelClicked = (_id) => {
            if (this.step === 'ls_click_meadow')
                this.advanceTo('ls_click_easy');
        };
        this.onDiffClicked = (diff) => {
            if (this.step === 'ls_click_easy' && diff === 'easy')
                this.advanceTo('ls_click_start');
        };
        this.onGameReady = () => {
        ...
        this.onBuildMode = (active, kind, _towerKind) => {
            if (this.step === 'game_press_1' && kind === 'tower') {
                this.resumeGame();
                this.advanceTo('game_place_tower');
            }
            if (this.step === 'game_press_4' && kind === 'wall') {
                this.resumeGame();
                this.advanceTo('game_place_walls');
            }
            if (this.step === 'game_exit_build' && !active) {
        ...
        this.onTowerPlaced = () => {
            if (this.step === 'game_place_tower')
                this.advanceTo('game_watch_tower', 2000);
        };
        ...
        this.onTowerSelected = () => {
            if (this.step === 'game_click_tower') {
                this.resumeGame();
                this.advanceTo('game_upgrade_tower');
            }
        };
        this.onTowerDeselected = () => {
            if (this.step === 'game_deselect_tower')
                this.advanceTo('game_done', 1500);
        };
        this.onTowerUpgraded = () => {
            if (this.step === 'game_upgrade_tower')
                this.advanceTo('game_deselect_tower', 1500);
        };
        this.onWallPlaced = () => {
            if (this.step === 'game_place_walls') {
                this.wallsPlaced++;
                if (this.wallsPlaced >= 3)
                    this.advanceTo('game_exit_build');
                else
                    this.showStep(); // update counter
            }
        };
    ...
    advanceTo(step, delayMs = 0) {
        if (delayMs > 0) {
            if (this.pendingStep)
                return; // already waiting for a delayed transition
    ...
    showStep() {
        ...
        switch (this.step) {
            ...
            case 'game_click_tower': {
                ...
                const gsTower = this.scene.get('Game');
    ...
    showClickPrompt(text, y, nextStep, nextDelay = 0) {
    ...
    cleanupContinueZone() {
        if (this.continueZone) {
            this.continueZone.destroy();
            this.continueZone = null;
        }
    }
    cleanupHudLabels() {
        for (const obj of this.hudLabels)
            obj.destroy();
        this.hudLabels = [];
        if (this.hudClickZone) {
            this.hudClickZone.destroy();
            this.hudClickZone = null;
        }
    }
    showPrompt(text, y) {
    ...
    update() {
        ...
        switch (this.step) {
            ...
            case 'game_stand_still':
                if (this.stepDelay === 0)
                    this.stepDelay = this.time.now + 7000; // show prompt for 7s
            ...
            case 'game_loot_coins':
                ...
                if (this.stepDelay === 0)
                    this.stepDelay = this.time.now + 10000;
                if (this.stepDelay > 0 && this.time.now > this.stepDelay) {
    ...
    spawnTutorialEnemies(gameScene, count) {
        if (!gameScene?.player)
            return;
        const px = gameScene.player.x;
        const py = gameScene.player.y;

...
export const DIFFICULTY_ORDER = ['easy', 'medium', 'hard', 'oneHP'];
...
export function saveMedal(levelId, diff) {
    const store = loadMedals();
    const key = String(levelId);
    if (!store[key])
        store[key] = { easy: false, medium: false, hard: false, oneHP: false };
    store[key][diff] = true;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(store));
}
export function hasMedal(store, levelId) {
    const medals = store[String(levelId)];
    if (!medals)
        return false;
    return Object.values(medals).some(v => v);
}
export function isLevelUnlocked(store, levelId) {
    if (levelId <= 1)
        return true;
    return hasMedal(store, levelId - 1);
}
export function totalMedals(store) {
    const s = store ?? loadMedals();
    let count = 0;
    for (const lvl of Object.values(s)) {
        for (const v of Object.values(lvl))
            if (v)
                count++;
    }
    return count;

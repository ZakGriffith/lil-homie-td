...
function pxIdx(w, x, y) { return (y * w + x) * 4; }
function pxEq(d, w, x1, y1, x2, y2) {
    const i = pxIdx(w, x1, y1), j = pxIdx(w, x2, y2);
    return d[i] === d[j] && d[i + 1] === d[j + 1] && d[i + 2] === d[j + 2] && d[i + 3] === d[j + 3];
}
function pxCopy(s, sw, sx, sy, d, dw, dx, dy) {
...
function scale2x(src, dst, w, h) {
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            ...
            const ox = x * 2, oy = y * 2;
            if (ca && !cd && !ab)
                pxCopy(src, w, x, ay, dst, dw, ox, oy);
            else
                pxCopy(src, w, x, y, dst, dw, ox, oy);
            if (ab && !ca && !bd)
                pxCopy(src, w, bx, y, dst, dw, ox + 1, oy);
            else
                pxCopy(src, w, x, y, dst, dw, ox + 1, oy);
            if (cd && !bd && !ca)
                pxCopy(src, w, cx, y, dst, dw, ox, oy + 1);
            else
                pxCopy(src, w, x, y, dst, dw, ox, oy + 1);
            if (bd && !ab && !cd)
                pxCopy(src, w, bx, y, dst, dw, ox + 1, oy + 1);
            else
                pxCopy(src, w, x, y, dst, dw, ox + 1, oy + 1);
        }
    }
}
function makeCanvas(size, draw) {
    ...
    logCanvas.width = size;
    logCanvas.height = size;
    ...
    outCanvas.width = physSize;
    outCanvas.height = physSize;
...
function line(put, x0, y0, x1, y1, c) {
    let x = x0, y = y0;
...
function ellipse(put, cx, cy, rx, ry, c) {
    for (let y = -ry; y <= ry; y++)
        for (let x = -rx; x <= rx; x++)
            if ((x * x) / (rx * rx) + (y * y) / (ry * ry) <= 1)
                put(cx + x, cy + y, c);
}
function flashOverlay(put, size, within) {
    ...
    return (put) => {
        ...
        for (let dy = -1; dy <= 1; dy++)
            for (let dx = -6; dx <= 6; dx++)
                if ((dx * dx) / 36 + (dy * dy) / 1.5 <= 1)
                    put(cx + dx, 28 + dy, P.shadow);
        // ----- legs
        ...
        if (frame === 'hit') {
            for (let y = 5; y < 30; y++) {
                for (let x = 4; x < 28; x++) {
                    ...
                    if (y >= headCy - 4 && y <= 29 && x >= cx - 8 && x <= cx + 8)
                        put(x, y, P.white);
                }
            }
        }
    ...
    return (put) => {
        ...
        for (let dy = -1; dy <= 1; dy++)
            for (let dx = -8; dx <= 8; dx++)
                if ((dx * dx) / 64 + (dy * dy) / 1.5 <= 1)
                    put(16 + dx, 29 + dy, P.shadow);
        // feet (heavy stompers)
        let footY = 0;
        if (f === 'move1')
            footY = -1;
...
function drawEnemyDeer(f) {
    return (rawPut) => {
        ...
        const legY = cy + 8;
        if (phase === 0) {
            rect(put, 11, legY, 2, 7, bodyD);
            rect(put, 15, legY, 2, 7, bodyD);
            rect(put, 19, legY, 2, 7, bodyD);
            rect(put, 22, legY, 2, 7, bodyD);
        }
        else if (phase === 1) {
            rect(put, 10, legY, 2, 8, bodyD);
            rect(put, 15, legY, 2, 6, bodyD);
            rect(put, 18, legY, 2, 8, bodyD);
            rect(put, 23, legY, 2, 6, bodyD);
        }
        else {
            rect(put, 12, legY, 2, 6, bodyD);
            rect(put, 15, legY, 2, 8, bodyD);
            rect(put, 20, legY, 2, 6, bodyD);
            rect(put, 22, legY, 2, 8, bodyD);
        }
        // Hooves
        ...
        // Nose
        put(4, cy + 1, P.outline);
        put(5, cy + 1, P.outline);
        // Eye — red (corrupted)
        put(7, cy, '#ff3030');
        put(8, cy, '#ff3030');
        put(7, cy, '#aa0000');
...
// ==================================================================
function drawEnemyInfectedBasic(f) {
    return (put) => {
        if (f.startsWith('die')) {
            const step = parseInt(f.slice(3));
            const r = 8 - step * 2;
...
// ==================================================================
function drawEnemyInfectedHeavy(f) {
    return (put) => {
        if (f.startsWith('die')) {
            const step = parseInt(f.slice(3));
            const r = 10 - step * 2;
...
function drawEnemyToad(f) {
    return (put) => {
        ...
        if (f === 'hop0') {
            ...
            put(6, cy + 7, greenD);
            put(25, cy + 7, greenD); // webbed toes
        }
        ...
        put(20, cy - 5, '#e0ff40');
        put(21, cy - 5, '#e0ff40');
        put(12, cy - 4, P.outline);
        put(20, cy - 4, P.outline);
        // Mouth
        if (isAtk) {
            // Open mouth — spitting
...
function drawEnemyWolf(f) {
    return (put) => {
        ...
        if (f === 'move1' || f === 'move3')
            tailY = 1;
        put(6, 14 + tailY, bodyD);
        put(5, 13 + tailY, bodyM);
        put(4, 12 + tailY, body);
        put(4, 11 + tailY, bodyL);
        put(5, 11 + tailY, body);
        // hind legs
        let footY = 0;
        if (f === 'move1')
    ...
    return (put) => {
        ...
        for (let sx = -10; sx <= 10; sx++)
            for (let sy = -1; sy <= 1; sy++)
                if ((sx * sx) / 100 + (sy * sy) <= 1)
                    put(cx + sx, 29 + sy, P.shadow);
        ...
        for (let yy = -7; yy <= 7; yy++)
            for (let xx = -11; xx <= 11; xx++)
                if ((xx * xx) / 121 + (yy * yy) / 49 <= 1)
                    put(cx + xx, by + yy, o);
        for (let yy = -6; yy <= 6; yy++)
            for (let xx = -10; xx <= 10; xx++)
                if ((xx * xx) / 100 + (yy * yy) / 36 <= 1)
                    put(cx + xx, by + yy, furD);
        for (let yy = -5; yy <= 5; yy++)
            for (let xx = -9; xx <= 9; xx++)
                if ((xx * xx) / 81 + (yy * yy) / 25 <= 1)
                    put(cx + xx, by + yy, fur);
        // Shoulder hump
        ...
        for (let yy = 0; yy <= 3; yy++)
            for (let xx = -6; xx <= 6; xx++)
                if ((xx * xx) / 36 + (yy * yy) / 9 <= 1)
                    put(cx - d + xx, by + 2 + yy, belly);
...
function extractBearFrames(scene) {
...
function drawEnemyCrow(f) {
    return (put) => {
        ...
        // Attack: beak open
        if (f === 'atk0' || f === 'atk1') {
            put(15, 14 + bob, '#c8a028');
            put(17, 14 + bob, '#c8a028');
            if (f === 'atk1')
                put(16, 16 + bob, '#400808');
        }
    };
}
// ==================================================================
...
function drawEnemyBat(f) {
    return (put) => {
        ...
        // Fangs
        if (f === 'atk0' || f === 'atk1') {
            put(15, 14 + bob, P.white);
            put(17, 14 + bob, P.white);
            if (f === 'atk1') {
                put(15, 15 + bob, P.white);
                put(17, 15 + bob, P.white);
            }
        }
        else {
            put(15, 14 + bob, '#e0e0e0');
            put(17, 14 + bob, '#e0e0e0');
        }
    };
}
// ==================================================================
...
function drawEnemyDragonfly(f) {
    return (put) => {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            put(16, 10 + i + bob, c);
            if (i < 6) {
                put(15, 10 + i + bob, bodyD);
                put(17, 10 + i + bob, bodyD);
            }
        }
        // Head
...
function drawFogPhantomBody(put, opts) {
    const cx = 32;
    ...
    for (let dy = -2; dy <= 2; dy++)
        for (let dx = -14; dx <= 14; dx++)
            if ((dx * dx) / 196 + (dy * dy) / 5 <= 1)
                put(cx + dx, 58 + dy, P.shadow);
    ...
    for (let t = 0; t < 8; t++) {
        ...
        for (let i = 0; i < tendrilLen; i++) {
            ...
            if (fade > 0.3)
                put(Math.round(x), Math.round(y), col.wisp);
        }
    }
...
function drawFogPhantomDie(put, step) {
    const cx = 32, cy = 30;
...
function drawTowerBase(put) {
    ...
    for (let dy = -5; dy <= 5; dy++)
        for (let dx = -30; dx <= 30; dx++)
            if ((dx * dx) / 900 + (dy * dy) / 25 <= 1)
                put(cx + dx, 60 + dy, P.shadow);
    ...
    // Bottom edge
    for (let x = -29; x <= 29; x++)
        put(cx + x, faceBot, P.outline);
    // Stone block seams on front
    for (let row = 0; row < 5; row++) {
        const by = faceTop + 2 + row * 6;
        if (by >= faceBot - 1)
            break;
        for (let x = -faceHW + 1; x < faceHW; x++)
            put(cx + x, by, P.stoneD);
        const off = row % 2 === 0 ? 0 : 6;
        for (let vx = -faceHW + 3 + off; vx < faceHW; vx += 11) {
            for (let dy = 0; dy < 6 && by + dy < faceBot; dy++)
                put(cx + vx, by + dy, P.stoneD);
        }
    }
    // Arrow slit on front
    ...
    put(cx - 3, faceBot - 10, P.stoneM);
    put(cx + 2, faceBot - 10, P.stoneM);
    // Door arch
    put(cx - 2, faceBot - 11, P.stoneM);
    put(cx + 1, faceBot - 11, P.stoneM);
...
// ------------------------------------------------------------------
function drawTowerArcher(put) {
    const cx = 16;
    // Shadow
    for (let dy = -1; dy <= 1; dy++)
        for (let dx = -5; dx <= 5; dx++)
            if ((dx * dx) / 25 + (dy * dy) / 1.5 <= 1)
                put(cx + dx, 27 + dy, P.shadow);
    // Legs
    ...
    for (let y = -4; y <= -1; y++)
        for (let x = -4; x <= 4; x++)
            if (x * x + y * y <= 16)
                put(hx + x, hy + y, P.hood);
    // Hood point
    put(hx, hy - 5, P.hoodD);
    put(hx - 1, hy - 5, P.hoodD);
    put(hx, hy - 6, P.outline);
    // Hood highlight
    put(hx - 2, hy - 3, P.tunic);
    put(hx - 1, hy - 4, P.tunic);
    // Face
    ...
    return (put) => {
        ...
        for (let dy = -4; dy <= 4; dy++)
            for (let dx = -12; dx <= 12; dx++)
                if ((dx * dx) / 144 + (dy * dy) / 20 <= 1)
                    put(cx + dx, cy + 7 + dy, P.shadow);
...
function drawWall(mask, damaged) {
    return (put) => {
        const S = 32;
        ...
        for (let y = t; y <= bTop; y++) {
            for (let x = l; x <= r; x++) {
                ...
                else if (py < 0.12 && !hasN)
                    c = TL;
                else if (py < 0.25)
                    c = TL;
                else if (px < 0.08 && !hasW)
                    c = TL;
                else if (py > 0.85)
                    c = TD;
                else if (px > 0.92 && !hasE)
                    c = TD;
                else if (py < 0.5)
                    c = T;
                else
        ...
        for (let row = 0; row < 4; row++) {
            ...
            for (let vx = l + 4 + off; vx <= r - 2; vx += 10) {
                for (let dy = 1; dy < rowH && sy + dy <= bTop - 1; dy++)
                    put(vx, sy + dy, seamC);
            }
        }
        ...
        if (bBot - bTop > 3) {
            ...
            for (let vx = l + 6; vx <= r - 2; vx += 10) {
                for (let y = bTop + 2; y < fMidY; y++)
                    put(vx, y, fSeamC);
            }
            for (let vx = l + 11; vx <= r - 2; vx += 10) {
                for (let y = fMidY + 1; y < bBot - 1; y++)
                    put(vx, y, fSeamC);
            }
        }
        if (damaged) {
...
function drawArrow(frame) {
    return (put) => {
        ...
        put(18, cy, P.steelD);
        // head outline
        put(15, cy - 2, P.outline);
        put(15, cy + 2, P.outline);
        put(17, cy - 2, P.outline);
        put(17, cy + 2, P.outline);
        put(19, cy, P.outline);
    ...
    return (put) => {
        ...
        for (let dy = -r; dy <= r; dy++) {
            for (let dx = -r; dx <= r; dx++) {
                ...
                let color;
                if (dist < 0.4)
                    color = '#4a4a54'; // lighter center
                else if (dist < 0.7)
                    color = '#333340'; // mid
                else
                    color = '#1e1e28'; // dark edge
...
function drawBoulder(frame) {
    return (put) => {
        ...
        put(cx - 3, cy - 3, '#aaa098');
        put(cx - 2, cy - 4, '#b0a898');
        put(cx - 4, cy - 2, '#9a9088');
        if (frame === 0)
            put(cx - 3, cy - 4, '#c0b8a8');
        else
            put(cx - 2, cy - 3, '#c0b8a8');
        // Moss patches
        put(cx + 2, cy - 3, P.leafM);
        put(cx + 3, cy - 2, P.leaf);
        put(cx - 1, cy + 3, P.leafM);
        put(cx + 1, cy + 4, P.leafD);
        // Cracks
        put(cx + 1, cy, '#3a3028');
        put(cx + 1, cy + 1, '#3a3028');
...
function drawCoin(frame, tier = 'gold') {
...
function drawBoulderImpact(frame) {
    return (put) => {
        const cx = 16, cy = 16;
...
function drawDeathBurst(frame) {
    return (put) => {
        const cx = 16, cy = 16;
        const r = 3 + frame * 2;
        if (frame < 3)
            disc(put, cx, cy, r, P.sparkL);
...
function wnoise(wx, wy, scale) {
    const hash = (a, b) => {
        const n = ((a * 12289 + b * 51749 + 71) * 2654435761) >>> 0;
        return (n & 0xffff) / 0xffff;
    };
    const sm = (t) => t * t * (3 - 2 * t);
    const oneOctave = (x, y, s) => {
 ...
 */
function precomputeNoise(tileX, tileY, offsetX, offsetY, scale, step = 4) {
    const size = 32;
...
function riverVerticalCenterX(worldPy) {
    return -4 * 32
...
export function riverHorizontalCenterY(worldPx) {
    return -6 * 32
...
export function riverCenterPx(riverIdx, worldPy) {
    if (riverIdx === 0)
        return riverVerticalCenterX(worldPy);
...
function riverPixelKind(worldPx, worldPy) {
 ...
 */
export function getRiverTileGrid(tileX, tileY) {
...
function getRocksNear(basePx, basePy) {
    const rocks = [];
    const margin = 12;
    ...
    for (let wy = basePy - margin; wy < basePy + 32 + margin; wy += 5) {
        ...
        if (!inWater(lx, ly))
            rocks.push({ wx: lx, wy: ly, r: radius, shade });
        if (!inWater(rx, ry))
            rocks.push({ wx: rx, wy: ry, r: radius, shade });
    }
    ...
    for (let wx = basePx - margin; wx < basePx + 32 + margin; wx += 5) {
        ...
        if (!inWater(tx, ty))
            rocks.push({ wx: tx, wy: ty, r: radius, shade });
        if (!inWater(bx, by))
            rocks.push({ wx: bx, wy: by, r: radius, shade });
    }
    return rocks;
}
...
function drawGroundRiver(tileX, tileY) {
    return (put, putRGB) => {
        const basePx = tileX * 32;
        const basePy = tileY * 32;
        ...
        const writeRGB = putRGB ?? ((x, y, r, g, b) => {
        ...
        for (let py = 0; py < 32; py++) {
            ...
            for (let px = 0; px < 32; px++) {
                ...
                if (kind === 1) {
                    ...
                    bg += ripple * 3;
                    bb += ripple * 5;
                    writeRGB(px, py, br < 0 ? 0 : br > 255 ? 255 : (br + 0.5) | 0, bg < 0 ? 0 : bg > 255 ? 255 : (bg + 0.5) | 0, bb < 0 ? 0 : bb > 255 ? 255 : (bb + 0.5) | 0);
                }
                else if (kind === 2) {
...
function drawGroundWorld(tileX, tileY) {
    return (put) => {
        ...
        const shadeHex = shades.map(([r, g, b]) => '#' + r.toString(16).padStart(2, '0') + g.toString(16).padStart(2, '0') + b.toString(16).padStart(2, '0'));
...
function drawGroundCastle(tileX, tileY) {
    return (put) => {
        let s = ((tileX * 73856093 + tileY * 19349669) >>> 0) % 2147483647;
        const rnd = () => { s = (s * 16807) % 2147483647; return s / 2147483647; };
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if (n > 0.45)
                put(sx, sy, rnd() > 0.5 ? '#6a747e' : '#4a5462');
        }
        ...
        if (rnd() < 0.06) {
            ...
            if (pattern === 0) {
                ...
                put(bx + 2, by + 1, bone);
                put(bx + 2, by + 2, bone);
                put(bx + 3, by + 2, boneHi);
                put(bx - 1, by, boneLo);
                put(bx + 4, by + 2, boneLo);
            }
            else if (pattern === 1) {
                ...
                put(bx + 2, by, bone);
                put(bx, by + 1, boneHi);
                put(bx + 1, by + 1, '#2a2228');
                put(bx + 2, by + 1, boneHi);
                put(bx, by + 2, boneLo);
                put(bx + 1, by + 2, boneLo);
                put(bx + 2, by + 2, boneLo);
            }
            else if (pattern === 2) {
                ...
                put(bx + 3, by, bone);
                put(bx, by + 2, boneLo);
                put(bx + 1, by + 1, boneLo);
                put(bx + 2, by + 1, bone);
                put(bx + 3, by + 2, boneLo);
            }
            else if (pattern === 3) {
...
function drawGroundForest(tileX, tileY) {
    return (put) => {
        ...
        if (rnd() < 0.30) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (dn > 0.68) {
                    ...
                    put(rx, ry, col);
                    if (rnd() > 0.5)
                        put(rx + 1, ry, col); // sometimes 2px wide
                }
            }
        }
        // Rare grass tuft (~8%)
...
function drawGroundInfected(tileX, tileY) {
    return (put) => {
        ...
        if (rnd() < 0.03) {
            ...
            for (let dy = 0; dy < sz; dy++) {
                for (let dx = 0; dx < sz; dx++) {
                    if (rnd() > 0.4)
                        put(px0 + dx, py0 + dy, rnd() > 0.5 ? '#40e060' : '#30b848');
                }
            }
        }
...
function drawTreeClusterCanvas(patternIdx) {
...
function drawInfectedPlantCanvas(patternIdx) {
    const pattern = TREE_PATTERNS[patternIdx];
    const T = 32;
    const pad = 30;
    const cw = pattern.w * T + pad * 2;
...
function drawBossBody(put, opts) {
    const cx = 32;
    ...
    for (let dy = -2; dy <= 2; dy++)
        for (let dx = -26; dx <= 26; dx++)
            if ((dx * dx) / 676 + (dy * dy) / 5 <= 1)
                put(cx + dx, 59 + dy, P.shadow);
    // stubby legs (4)
    ...
    for (let y = -20; y <= -5; y++)
        for (let x = -20; x <= 20; x++)
            if (x * x + y * y <= 400)
                put(cx + x, baseCy + y, col.b);
    // highlight arc upper-left
    for (let y = -20; y <= -10; y++)
        for (let x = -18; x <= -2; x++)
            if (x * x + y * y <= 324)
                put(cx + x, baseCy + y, col.m);
    for (let y = -18; y <= -14; y++)
        for (let x = -10; x <= -4; x++)
            if (x * x + y * y <= 256)
                put(cx + x, baseCy + y, col.l);
    // pale swollen underbelly (lower half)
    for (let y = 4; y <= 22; y++)
        for (let x = -20; x <= 20; x++)
            if (x * x + y * y <= 476)
                put(cx + x, baseCy + y, P.belly);
    for (let y = 10; y <= 22; y++)
        for (let x = -17; x <= 17; x++)
            if (x * x + y * y <= 400)
                put(cx + x, baseCy + y, P.bellyM);
    // segmentation lines
    for (let x = -17; x <= 17; x++) {
        if (Math.abs(x) < 16)
            put(cx + x, baseCy + 8, P.bellyD);
        if (Math.abs(x) < 14)
            put(cx + x, baseCy + 14, P.bellyD);
        if (Math.abs(x) < 10)
            put(cx + x, baseCy + 19, P.bellyD);
    }
...
function drawBossDie(put, step) {
    ...
    // central flash
    if (step < 2)
        disc(put, cx, cy, 6, P.sparkL);
}
function drawRamBody(put, opts) {
    const cx = 32;
    const baseCy = 34 + (opts.bob ?? 0) + (opts.rearUp ? -2 : 0);
    const col = {
        out: opts.flash ? P.white : P.outline,
    ...
    for (let y = -19; y <= -3; y++)
        for (let x = -18; x <= 18; x++)
            if (x * x + y * y <= 361)
                put(cx + x, baseCy + y, wc.d);
    for (let y = -17; y <= -5; y++)
        for (let x = -16; x <= 16; x++)
            if (x * x + y * y <= 289)
                put(cx + x, baseCy + y, wc.b);
    // Wool highlight (upper-left)
    for (let y = -16; y <= -8; y++)
        for (let x = -14; x <= -2; x++)
            if (x * x + y * y <= 225)
                put(cx + x, baseCy + y, wc.l);
    // Wool texture lumps
    ...
    for (let y = 4; y <= 19; y++)
        for (let x = -16; x <= 16; x++)
            if (x * x + y * y <= 380)
                put(cx + x, baseCy + y, P.ramBelly);
    for (let y = 10; y <= 19; y++)
        for (let x = -12; x <= 12; x++)
            if (x * x + y * y <= 340)
                put(cx + x, baseCy + y, P.ramL);
    // --- Shoulder hump ---
    ...
    // Nose
    put(hx + 9, hy + 1, col.out);
    put(hx + 10, hy + 1, col.out);
    put(hx + 9, hy + 2, col.out);
    put(hx + 10, hy + 2, col.out);
    // Mouth
    ...
    // Eye — angry amber
    put(hx + 2, hy - 2, '#ffcc20');
    put(hx + 3, hy - 2, '#ffcc20');
    put(hx + 2, hy - 1, '#ffaa00');
    put(hx + 3, hy - 1, '#ffcc20');
...
// ==================================================================
function drawInfectedBossBody(put, opts) {
    const cx = 32;
    const baseCy = 34 + (opts.bob ?? 0) + (opts.rearUp ? -2 : 0);
    const col = {
        out: opts.flash ? P.white : P.outline,
    ...
    for (let y = 4; y <= 22; y++)
        for (let x = -20; x <= 20; x++)
            if (x * x + y * y <= 476)
                put(cx + x, baseCy + y, bellyCol);
    for (let y = 10; y <= 22; y++)
        for (let x = -17; x <= 17; x++)
            if (x * x + y * y <= 400)
                put(cx + x, baseCy + y, bellyColM);
    // segmentation lines
    for (let x = -17; x <= 17; x++) {
        if (Math.abs(x) < 16)
            put(cx + x, baseCy + 8, bellyColD);
        if (Math.abs(x) < 14)
            put(cx + x, baseCy + 14, bellyColD);
        if (Math.abs(x) < 10)
            put(cx + x, baseCy + 19, bellyColD);
    }
...
function drawInfectedBossDie(put, step) {
    const cx = 32, cy = 36;
    const r = Math.max(0, 24 - step * 5);
    if (r > 0) {
...
function drawInfectedBoss(frame) {
    return (put) => {
        switch (frame) {
...
function drawWendigoBody(put, opts) {
    const cx = 32;
    const bob = opts.bob ?? 0;
    const cy = 30 + bob;
    const phase = opts.phase ?? 0;
    ...
    for (let dx = -16; dx <= 16; dx++)
        for (let dy = -1; dy <= 1; dy++)
            if ((dx * dx) / 256 + (dy * dy) / 2 <= 1)
                put(cx + dx, 58 + dy, P.wGhostD);
    ...
    put(cx - 3, cy - 12, eyeCol);
    put(cx - 2, cy - 12, eyeHL);
    put(cx - 3, cy - 13, eyeDk);
    disc(put, cx + 3, cy - 12, 2, out);
    put(cx + 3, cy - 12, eyeCol);
    put(cx + 4, cy - 12, eyeHL);
    put(cx + 3, cy - 13, eyeDk);
    // Nose holes
    put(cx - 1, cy - 6, out);
    put(cx + 1, cy - 6, out);
...
function drawWendigoDie(put, step) {
    const cx = 32, cy = 30;
...
function drawIndicatorArrow() {
    return (put) => {
        ...
        // arrowhead
        put(cx + 5, cy - 3, P.stone);
        put(cx + 6, cy - 2, P.stone);
        put(cx + 7, cy - 1, P.stoneL);
        put(cx + 7, cy, P.stoneL);
        put(cx + 6, cy + 1, P.stone);
        put(cx + 5, cy + 2, P.stone);
        // fletching
        put(cx - 5, cy - 2, P.white);
        put(cx - 6, cy - 3, P.white);
        put(cx - 5, cy + 1, P.white);
        put(cx - 6, cy + 2, P.white);
    };
}
...
function drawIndicatorBoss() {
    return (put) => {
        ...
        // Eye sockets
        put(cx - 2, cy - 2, P.outline);
        put(cx - 2, cy - 1, P.outline);
        put(cx + 2, cy - 2, P.outline);
        put(cx + 2, cy - 1, P.outline);
        // Red eye glow
        put(cx - 2, cy - 2, '#ff3333');
        put(cx + 2, cy - 2, '#ff3333');
        // Nose
        put(cx, cy, P.outline);
        // Jaw
        ...
        // Teeth
        put(cx - 2, cy + 2, P.outline);
        put(cx, cy + 2, P.outline);
        put(cx + 2, cy + 2, P.outline);
        put(cx - 2, cy + 3, P.outline);
        put(cx, cy + 3, P.outline);
        put(cx + 2, cy + 3, P.outline);
    };
}
...
export function createGroundChunk(scene, chunkX, chunkY, chunkSize, tileSize, biome = 'grasslands') {
    ...
    for (let ty = 0; ty < chunkSize; ty++) {
        for (let tx = 0; tx < chunkSize; tx++) {
            ...
            const put = (x, y, col) => {
                ...
                if (px < 0 || py < 0 || px >= tileSize || py >= tileSize)
                    return;
...
function drawPhantomQueenBody(put, opts) {
    const cx = 32;
    const bob = opts.bob ?? 0;
    const baseCy = 30 + bob;
    ...
    for (let y = baseCy + 2; y < baseCy + 24; y++) {
        ...
        for (let dx = -hw; dx <= hw; dx++) {
            ...
            if (t > 0.7 && ((dx + y) % 3 === 0 || Math.abs(dx) > hw - 2))
                continue;
            if (t > 0.85 && (dx + y) % 2 === 0)
                continue;
            let c;
    ...
    for (let side = -1; side <= 1; side += 2) {
        for (let y = baseCy - 16; y < baseCy + 8; y++) {
            ...
            if (y < baseCy - 8)
                put(baseX - side, y, col.hair);
        }
    }
    // Crown
    const crownY = baseCy - 19;
    for (let dx = -5; dx <= 5; dx++)
        put(cx + dx, crownY + 2, col.crown);
    for (let dx = -4; dx <= 4; dx++)
        put(cx + dx, crownY + 1, col.crownL);
    // Crown points
    put(cx - 4, crownY, col.crown);
    put(cx - 3, crownY - 1, col.crownL);
    put(cx, crownY, col.crown);
    put(cx, crownY - 1, col.crownL);
    put(cx + 4, crownY, col.crown);
    put(cx + 3, crownY - 1, col.crownL);
    // Jewels
...
function drawPhantomQueenDie(put, step) {
    const cx = 32, cy = 30;
    ...
    if (step < 2)
        disc(put, cx, cy, 4, '#aad0ff');
}
function drawPhantomQueen(frame) {
    return (put) => {
        switch (frame) {
...
function drawCastleDragonBody(put, opts) {
    const bob = opts.bob ?? 0;
    ...
    for (let dy = -2; dy <= 2; dy++)
        for (let dx = -16; dx <= 16; dx++)
            if ((dx * dx) / 256 + (dy * dy) / 4 <= 1)
                put(32 + dx, 59 + dy, P.shadow);
    ...
    for (let i = 0; i < 22; i++) {
        ...
        if (tr > 1)
            disc(put, tx, ty, tr - 1, col.b);
    }
    // Tail spikes
    put(0, 36 + by, col.hornD);
    put(1, 35 + by, col.horn);
    put(0, 38 + by, col.hornD);
    put(1, 39 + by, col.horn);
    ...
    for (let i = 0; i < 22; i++) {
        ...
        if (i % 3 === 0)
            put(4 + i, wy + by, col.wingL); // membrane veins
    }
    // Wing bone
    ...
    // Nostrils
    put(57, 17 + by, col.nostril);
    put(57, 19 + by, col.nostril);
    // Jaw
    rect(put, 40, 22 + by, 16, 3, col.m);
    rect(put, 42, 22 + by, 12, 2, col.b);
    // Teeth
    for (let x = 42; x < 54; x += 3) {
        put(x, 22 + by, col.tooth);
        put(x, 23 + by, col.toothD);
    }
    // Brow ridge
    ...
    put(41, 3 + by, col.hornL);
    put(49, 3 + by, col.hornL);
    ...
    if (opts.pockets != null) {
        ...
        for (let t = 0; t < 6; t++) {
            ...
            put(fx, fy, col.fire);
            if (p >= 2)
                put(fx + 1, fy, col.fireL);
            if (p >= 4)
                disc(put, fx, fy, 2, col.fireL);
        }
    }
}
function drawCastleDragonDie(put, step) {
    const cx = 32, cy = 34;
    const r = Math.max(0, 14 - step * 3);
    if (r > 0) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        if (i % 2 === 0)
            put(x, y - 1, step < 3 ? '#ff6020' : '#c07040');
    }
    // Central flame dying
    if (step < 2)
        disc(put, cx, cy, 4, '#ff8020');
    if (step < 1)
        disc(put, cx, cy, 6, '#ffaa40');
}
function drawCastleDragon(frame) {
    return (put) => {
        switch (frame) {
...
// ==================================================================
function drawDragonFireball(frame) {
    return (put) => {
        const cx = 16, cy = 16, r = 7;
        ...
        for (let dy = -(r + 1); dy <= r + 1; dy++) {
            for (let dx = -(r + 1); dx <= r + 1; dx++) {
                ...
                if (d2 > (r + 1) * (r + 1) || d2 <= r * r)
                    continue;
                if (Math.sqrt(d2) > r && Math.sqrt(d2) <= r + 1.2)
                    put(cx + dx, cy + dy, '#8a2010');
            }
        }
...
// ==================================================================
function drawDragonFireExplosion(frame) {
    return (put) => {
        const cx = 16, cy = 16;
        ...
        for (let dy = -outerR; dy <= outerR; dy++) {
            for (let dx = -outerR; dx <= outerR; dx++) {
                ...
                if (norm < 0.3)
                    ...
                    color = '#c04010';
                if (alpha < 0.6 && norm > 0.5)
                    continue; // fade outer edges
                put(cx + dx, cy + dy, color);
            }
        }
        // Flying ember chunks
...
function drawEnemySkeleton(f) {
    return (rawPut) => {
        ...
        // Eye sockets
        put(14, 6 + bob, P.outline);
        put(15, 6 + bob, P.outline);
        put(17, 6 + bob, P.outline);
        put(18, 6 + bob, P.outline);
        // Nose
        put(16, 7 + bob, boneDD);
        // Jaw
        ...
        // Left leg
        put(14, 18 + bob, boneD);
        put(14, 19 + bob + legOff, boneD);
        put(14, 20 + bob + legOff, boneD);
        put(14, 21 + bob + legOff, boneD);
        put(13, 22 + bob + legOff, boneDD);
        put(14, 22 + bob + legOff, boneDD);
        // Right leg
        put(18, 18 + bob, boneD);
        put(18, 19 + bob - legOff, boneD);
        put(18, 20 + bob - legOff, boneD);
        put(18, 21 + bob - legOff, boneD);
        put(17, 22 + bob - legOff, boneDD);
        put(18, 22 + bob - legOff, boneDD);
    };
}
// ==================================================================
//  WARLOCK (32x32) — dark robed magic caster with glowing purple eyes
// ==================================================================
function drawEnemyWarlock(f) {
    return (rawPut) => {
        const put = f.startsWith('die') ? rawPut : mirrorX(rawPut);
        if (f.startsWith('die')) {
            const step = parseInt(f.slice(3));
        ...
        put(11, 8 + bob, '#5a3a1a');
        put(11, 9 + bob, '#5a3a1a');
        put(11, 10 + bob, '#5a3a1a');
        put(11, 11 + bob, '#5a3a1a');
        put(11, 12 + bob, '#5a3a1a');
        put(11, 13 + bob, '#5a3a1a');
        put(11, 14 + bob, '#5a3a1a');
        put(11, 15 + bob, '#5a3a1a');
        put(11, 16 + bob, '#5a3a1a');
        put(11, 17 + bob, '#5a3a1a');
        // Crystal on top
        put(11, 6 + bob, glow);
        put(11, 5 + bob, glowL);
        put(10, 6 + bob, glow);
...
function drawEnemyGolem(f) {
    return (rawPut) => {
        ...
        put(23, 18 + bob - atkSwing, stoneD);
        put(24, 18 + bob - atkSwing, stoneD);
        put(25, 18 + bob - atkSwing, stoneD);
        // Arm runes
        put(8, 15 + bob, rune);
...
function drawEnemyShadowImp(f) {
    return (rawPut) => {
        ...
        put(18, 12 + bob, eyes);
        // Eye glow
        put(14, 11 + bob, '#ffaa44');
        put(18, 11 + bob, '#ffaa44');
        // Red grin
        put(14, 14 + bob, grin);
        put(15, 14 + bob, grin);
        put(16, 14 + bob, grin);
        ...
        // Small legs
        const legOff = [0, 1, 0, -1][phase];
        put(14, 22 + bob + legOff, bodyL);
        put(14, 23 + bob + legOff, bodyL);
        put(13, 24 + bob + legOff, body);
        put(18, 22 + bob - legOff, bodyL);
        put(18, 23 + bob - legOff, bodyL);
...
function drawEnemyCastleBat(f) {
    return (put) => {
        ...
        for (let dy = 0; dy <= 3; dy++)
            for (let dx = -2; dx <= 2; dx++)
                if ((dx * dx) / 4 + (dy * dy) / 9 <= 1)
                    put(16 + dx, 18 + bob + dy, bodyL);
        // Head
        disc(put, 16, 11 + bob, 3, bodyL);
        ...
        put(18, 11 + bob, eyeC);
        // Eye glow
        put(14, 10 + bob, '#ff4040');
        put(18, 10 + bob, '#ff4040');
        // Fangs
        put(15, 14 + bob, fang);
        put(17, 14 + bob, fang);
        if (f === 'atk0' || f === 'atk1') {
...
export function generateAllArt(scene) {
    if (artGenerated)
        return;
    artGenerated = true;
    // Player
    ...
    for (const { k, f } of pFrames)
        add(scene, k, makeCanvas(32, drawPlayer(f)));
    ...
    // Enemies
    const eFrames = ['move0', 'move1', 'move2', 'move3', 'atk0', 'atk1', 'hit', 'die0', 'die1', 'die2', 'die3'];
    for (const f of eFrames)
        add(scene, `eb_${f}`, makeCanvas(32, drawEnemyBasic(f)));
    for (const f of eFrames)
        add(scene, `eh_${f}`, makeCanvas(32, drawEnemyHeavy(f)));
    for (const f of eFrames)
        add(scene, `esnk_${f}`, makeCanvas(32, drawEnemySnake(f)));
    for (const f of eFrames)
        add(scene, `erat_${f}`, makeCanvas(32, drawEnemyRat(f)));
    for (const f of eFrames)
        add(scene, `eder_${f}`, makeCanvas(32, drawEnemyDeer(f)));
    for (const f of eFrames)
        add(scene, `eib_${f}`, makeCanvas(32, drawEnemyInfectedBasic(f)));
    for (const f of eFrames)
        add(scene, `eih_${f}`, makeCanvas(32, drawEnemyInfectedHeavy(f)));
    // Blighted Toad — uses its own frame set (idle + hop + atk + hit + die)
    const toadFrames = ['idle', 'hop0', 'hop1', 'hop2', 'hop3', 'atk0', 'atk1', 'hit', 'die0', 'die1', 'die2', 'die3'];
    for (const f of toadFrames)
        add(scene, `etd_${f}`, makeCanvas(32, drawEnemyToad(f)));
    // Toad glob projectile
    add(scene, 'tglob_0', makeCanvas(16, drawToadGlob('glob0')));
    add(scene, 'tglob_1', makeCanvas(16, drawToadGlob('glob1')));
    for (const f of eFrames)
        add(scene, `ew_${f}`, makeCanvas(32, drawEnemyWolf(f)));
    // Bear: extract frames from sprite sheet, strip grey bg, register as textures
    extractBearFrames(scene);
    for (const f of eFrames)
        add(scene, `es_${f}`, makeCanvas(32, drawEnemySpider(f)));
    // River flying enemies
    for (const f of eFrames)
        add(scene, `ecr_${f}`, makeCanvas(32, drawEnemyCrow(f)));
    for (const f of eFrames)
        add(scene, `ebt_${f}`, makeCanvas(32, drawEnemyBat(f)));
    for (const f of eFrames)
        add(scene, `edf_${f}`, makeCanvas(32, drawEnemyDragonfly(f)));
    for (const f of eFrames)
        add(scene, `emq_${f}`, makeCanvas(32, drawEnemyMosquito(f)));
    ...
    // Castle enemies
    for (const f of eFrames)
        add(scene, `esk_${f}`, makeCanvas(32, drawEnemySkeleton(f)));
    for (const f of eFrames)
        add(scene, `ewl_${f}`, makeCanvas(32, drawEnemyWarlock(f)));
    for (const f of eFrames)
        add(scene, `ego_${f}`, makeCanvas(32, drawEnemyGolem(f)));
    for (const f of eFrames)
        add(scene, `esi_${f}`, makeCanvas(32, drawEnemyShadowImp(f)));
    for (const f of eFrames)
        add(scene, `ecb_${f}`, makeCanvas(32, drawEnemyCastleBat(f)));
    for (const f of eFrames)
        add(scene, `ecrat_${f}`, makeCanvas(32, drawEnemyCastleRat(f)));
    ...
    const copyTex = (src, dst) => {
        if (scene.textures.exists(dst))
            scene.textures.remove(dst);
    ...
    for (let i = 0; i < 6; i++)
        add(scene, `coin_${i}`, makeCanvas(32, drawCoin(i, 'gold')));
    for (const tier of ['bronze', 'silver', 'gold']) {
        for (let i = 0; i < 6; i++)
            add(scene, `coin_${tier}_${i}`, makeCanvas(32, drawCoin(i, tier)));
    }
    // Effects
    for (let i = 0; i < 3; i++)
        add(scene, `fx_hit_${i}`, makeCanvas(32, drawHitSpark(i)));
    for (let i = 0; i < 5; i++)
        add(scene, `fx_death_${i}`, makeCanvas(32, drawDeathBurst(i)));
    for (let i = 0; i < 3; i++)
        add(scene, `fx_pop_${i}`, makeCanvas(32, drawCoinPop(i)));
    for (let i = 0; i < 5; i++)
        add(scene, `fx_boulder_${i}`, makeCanvas(32, drawBoulderImpact(i)));
    ...
    for (let i = 0; i < TREE_PATTERNS.length; i++)
        add(scene, `tree_cluster_${i}`, drawTreeClusterCanvas(i));
    // Infected plant cluster sprites (one per pattern)
    for (let i = 0; i < TREE_PATTERNS.length; i++)
        add(scene, `infected_plant_${i}`, drawInfectedPlantCanvas(i));
    ...
    for (const f of bossFrames)
        add(scene, `boss_${f}`, makeCanvas(64, drawBoss(f)));
    for (const f of bossFrames)
        add(scene, `ram_${f}`, makeCanvas(64, drawRam(f)));
    for (const f of bossFrames)
        add(scene, `iboss_${f}`, makeCanvas(64, drawInfectedBoss(f)));
    // Forest boss (Ent) textures
    for (const f of forestBossFrames)
        add(scene, `fboss_${f}`, makeCanvas(64, drawForestBoss(f)));
    // River boss (Fog Phantom) textures
    for (const f of bossFrames)
        add(scene, `rboss_${f}`, makeCanvas(64, drawFogPhantom(f)));
    // Castle boss (Phantom Queen) textures
    for (const f of bossFrames)
        add(scene, `cqboss_${f}`, makeCanvas(64, drawPhantomQueen(f)));
    // Castle boss (Castle Dragon) textures
    for (const f of bossFrames)
        add(scene, `cdboss_${f}`, makeCanvas(64, drawCastleDragon(f)));
...
export function registerAnimations(scene) {
    ...
    for (const tier of ['bronze', 'silver', 'gold']) {
        mk(`coin-${tier}-spin`, [`coin_${tier}_0`, `coin_${tier}_1`, `coin_${tier}_2`, `coin_${tier}_3`, `coin_${tier}_4`, `coin_${tier}_5`], 10, -1);
    }
    ...
    for (let vi = 0; vi < 5; vi++) {
        ...
        if (scene.textures.exists(`river_squig_${vi}`))
            scene.textures.remove(`river_squig_${vi}`);
